...
class Test_Selenium_Execution():  
    ...
    def py_wait(self,element,action,values):
        if "pywait" in element:
            self.wait_till(int(values))
        elif "expwait" in element:
            pass
    def saveimage(self,element,action,values):
        ...
        if "fail_error" not in str(element):
            ...
            self.driver.save_screenshot("screenshot/Date-"+str(dt_string2)+"/Fail_snaps/"+str(element)+" - "+str(fail_name)+" - "+str(dt_string)+".png")
    ...
    def old_initialise(self):
        ...
        if 5 == len(credential) and int(credential[3]) != 0:
            try:
                ...
                combodata = combo.find_elements_by_css_selector("[value]")
                comval = []
                for i in combodata:
    ...
    def fs_initialise(self):
        self.init_web()
        self.driver.get(credential[2])
        self.driver.find_element(By.CSS_SELECTOR, "[type='email']").send_keys(credential[0])
    ...
    def logs(self,exe_name):
        global logger
        form = logging.Formatter("%(asctime)s\t%(levelname)s\t"+str("File name\t"+str(exe_name)+"\tMessage\t%(message)s"))
                    ...
                    for ran in range(line_exe,len(exe_lines)):
                        ...
                        try:
                            ...
                            if len(code_line) == 2:
                                ...
                                continue
                            # var = str(code_line[0])
                            code_line[0] = str(code_line[0]).strip()
